...
def get_access_token(stop_event):
    ...
    while not stop_event.is_set():
        ...
        if present > limit:
            secrets = pd.read_csv("secrets2.csv")
            ...
            pd.DataFrame([response_data['access_token']],columns=["token"]).to_csv("access_token2.csv")
def headers2():
    script_directory = os.path.dirname(os.path.abspath(__file__))
    token = pd.read_csv(script_directory+"/access_token2.csv").iloc[0]["token"]
